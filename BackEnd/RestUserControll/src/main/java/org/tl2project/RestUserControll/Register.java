...
//Path: http://localhost/RestUserController/webapi/register
@Path("/register")
public class Register {
	// HTTP Get Method
	@GET 
	// Path: http://localhost/RestUserController/webapi/register/doregister
	@Path("/doregister")  
	// Produces JSON as response
	@Produces(MediaType.APPLICATION_JSON) 
	// Query parameters are parameters: http://localhost/RestUserController/webapi/register/doregister?email=pqrs&username=abc&password=xyz
	public String doRegister(@QueryParam("email") String email, @QueryParam("username") String uname, @QueryParam("password") String pwd){
		String response = "";
	...
	private int registerUser(String email, String uname, String pwd){
		...
		if(Utility.isNotNull(uname) && Utility.isNotNull(pwd)){
			try {
				if(DBConnection.insertUser(email, uname, pwd)){
